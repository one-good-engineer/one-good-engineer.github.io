...
- The automation form submits to the public Formspree endpoint `https://formspree.io/f/xnogenpm` via JSON, without opening email. CAPTCHA is disabled with the owner's approval (2026-09-09); Formshield is enabled and a `_gotcha` honeypot is included. Delivery settings and submissions are managed in Formspree. The request includes all diagnostic fields, email Reply-To when the contact is an email, and whitelisted UTM parameters. Success requires Formspree confirmation; failures retain entered data. A 15-second timeout and in-flight guard prevent a stuck form or duplicate clicks. No API secret or build-time environment variable is needed.
